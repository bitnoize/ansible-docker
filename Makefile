...
build: .build-6.4.0-bullseye

rebuild: export BUILD_OPTS := --pull --no-cache
rebuild: .build-6.4.0-bullseye

.build-6.4.0-bullseye:
	docker build $(BUILD_OPTS) \
		--build-arg ANSIBLE_VERSION=6.4.0 \
		-t "$(IMAGENAME):6.4.0-bullseye" \
...
push: .push-6.4.0-bullseye

.push-6.4.0-bullseye:
	docker push $(PUSH_OPTS) "$(IMAGENAME):6.4.0-bullseye"
	docker push $(PUSH_OPTS) "$(IMAGENAME):6-bullseye"
	docker push $(PUSH_OPTS) "$(IMAGENAME):latest"

#pull: export PULL_OPTS := ...
pull: .pull-6.4.0-bullseye

.pull-6.4.0-bullseye:
	docker pull $(PULL_OPTS) "$(IMAGENAME):6.4.0-bullseye"

...
build: .build-7.2.0-bullseye

rebuild: export BUILD_OPTS := --pull --no-cache
rebuild: .build-7.2.0-bullseye

.build-7.2.0-bullseye:
	docker build $(BUILD_OPTS) \
		--build-arg ANSIBLE_VERSION=7.2.0 \
		-t "$(IMAGENAME):7.2.0-bullseye" \
...
push: .push-7.2.0-bullseye

.push-7.2.0-bullseye:
	docker push $(PUSH_OPTS) "$(IMAGENAME):7.2.0-bullseye"
	docker push $(PUSH_OPTS) "$(IMAGENAME):7-bullseye"
	docker push $(PUSH_OPTS) "$(IMAGENAME):latest"

#pull: export PULL_OPTS := ...
pull: .pull-7.2.0-bullseye

.pull-7.2.0-bullseye:
	docker pull $(PULL_OPTS) "$(IMAGENAME):7.2.0-bullseye"
